<template>
  <div>
    ...
    <el-card class="user_center_container">
      <el-row class="center_row row1">
        <el-col :span="10" class="avatar_box">
          <img src="https://dss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=181718184,3275785172&fm=111&gp=0.jpg" alt="" width="120" height="120">
          <div>{{userInfo.name}}</div>
        </el-col>
        <el-col :span="14">
          <el-row>
            <el-col :span="12">
              <h3 style="font-size: 30px; margin: 30px 0 20px 0;">我的钱包</h3>
              <span><i class="el-icon-wallet ytext" style="margin-right: 10px;"></i></span>
              <span class="ytext">￥ {{balance}}</span>
            </el-col>
            <el-col :span="12"  style="border-left: 1px solid #F5F5F5">
              <span style="line-height: 200px" class="ytext cursor"><i class="el-icon-shopping-cart-full"></i> 我的购物车</span>
            </el-col>
          </el-row>
        </el-col>
      </el-row>
      <el-row class="center_row">
        <el-collapse accordion>
          <el-collapse-item>
            <template slot="title">
              <i class="el-icon-coin title_i"></i>充值
            </template>
            <el-form ref="form" :model="userInfo" label-width="70px" label-position="left">
              <el-form-item label="人民币">
                <el-input v-model="money" type="number" size="small"></el-input>
              </el-form-item>
              <el-button class="submit_btn" type="primary" size="small" @click="setTopUp()">充值</el-button>
            </el-form>
          </el-collapse-item>
          <el-collapse-item>
            <template slot="title">
              <i class="el-icon-s-order title_i"></i>查看订单
            </template>
            <el-row style="border-bottom: 2px solid #F5F5F5">
              <el-col class="text_center" :span="6">编号</el-col>
              <el-col class="text_center" :span="6">订单号</el-col>
              <el-col class="text_center" :span="6">状态</el-col>
              <el-col class="text_center" :span="6">价格</el-col>
            </el-row>
            <el-row>
          ...
          <el-collapse-item>
            <template slot="title">
              <i class="el-icon-user-solid title_i"></i>修改用户信息
            </template>
            <el-form ref="form" :model="userInfo" label-width="70px" label-position="left">
              ...
              <el-form-item label="密码">
                <el-input v-model="userInfo.passwd" size="small"></el-input>
              </el-form-item>
              <el-button class="submit_btn" type="primary" size="small" @click="setLogin()">修改</el-button>
            </el-form>
          </el-collapse-item>
          <el-collapse-item>
            <template slot="title">
              <i class="el-icon-location title_i"></i>添加地址
            </template>
            <el-form ref="form" :model="userInfo" label-width="70px" label-position="left">
              <el-form-item label="地区选择">
               <el-cascader
                ...
                clearable
                collapse-tags
                placeholder="请选择地址，可多选"></el-cascader>
              </el-form-item>
              <el-form-item label="详细地址">
                <el-input v-model="adressDetail" size="small"></el-input>
              </el-form-item>
              <el-button class="submit_btn" type="primary" size="small" @click="setLogin()">添加</el-button>
            </el-form>
          </el-collapse-item>
        </el-collapse>
      </el-row>
...
export default {
      ...
      operatingAreaOptions: [
        {
          ...
          areaName: '全国',
          disabled: false,
          child: [{ areaId: '100000', areaName: '全国' }]
        },
        {
          areaId: '110000',
          areaName: '北京市',
  ...
  created () {
    ...
    this.userInfo.passwd = sessionStorage.getItem('passwd')
  ...
  methods: {
    async getOrderList () {
      var that = this
      const { data: res } = await this.$http.post('queryOrders', {
        condition: {
    ...
    async setUserInfoChange () {
      const { data: res } = await this.$http.post('conditional_query_users', {
    ...
    async setTopUp () {
      const { data: res } = await this.$http.post('add_balance', {
        userId: this.userInfo.userId,
        balance: this.money
      })
      if (res.code !== '1') {
      ...
      this.balance = parseInt(this.money) + parseInt(this.balance)
